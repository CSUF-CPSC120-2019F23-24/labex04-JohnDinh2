...
int main()
{
  ...
  char my_char3 = 'z';
  my_char3 = my_char3 + 1;
  std::cout << "Char2: " << my_char3 << std::endl;/* The value is no longer alphabetic because there is no other letter after z,
  which would mean there is nothing in the alphabet to represent. */
  std::cout << std::endl; // blank line to separate output from the different exercises
  ...
  std::cout << ", after adding 1: " << std::dec << my_short2 //my_short2 became negative because it was past the maximum value for a short, and so overflowed.
    << std::hex << " (0x" << my_short2 << ")\n";

  std::cout << "Int (" << sizeof(my_short2) << " bytes): "
    << std::dec << my_int2 << std::hex << " (0x" << my_int2 << ')';//my_int2 did not go past the max value for an int, so it output the correct number.
  my_int2 = my_int2 + 1;
  ...
  std::cout << ", after adding 1: " << std::dec << my_short3//my_short3 shows 0 because it overflowed and so had to start from the lowest value possible for a short
    << std::hex << " (0x" << my_short3 << ")\n";

  std::cout << "Same value as int (" << sizeof(my_int3) << " bytes): "
    << std::dec << my_int3 << std::hex << " (0x" << my_int3 << ')';
  my_int3 = my_int3 + 1;
  std::cout << ", after adding 1: " << std::dec << my_int3//my_int3 has a far larger max value, and so portrays the proper integer after one was added.
    << std::hex << " (0x" << my_int3 << ")\n";
